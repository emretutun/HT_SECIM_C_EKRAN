...
namespace HT_SECIM_C_EKRAN.Data
{
    /// <summary>
    ...
    public static class DataService
    {
        ...
        /// <summary>
        /// Bir ildeki oylari BUYUKTEN KUCUGE sirali dondurur.
        ///
        /// Orani sifir olanlar listeye girmiyor: C ekrani sayfalarini
        /// "anlamli aday/parti kaldi mi" diye bu listeye bakarak atliyor,
        /// %0,00 alan bir partiyi ekrana basmanin anlami yok.
        ///
        /// Her cagrida yeni liste uretiyor; donus motoru sayfa basina bir
        /// kez cagiriyor, sayfa icinde tekrar tekrar cagirmayin.
        /// </summary>
        public static List<Oy> SiraliOylar(string secimKod, int plaka)
        {
            List<Oy> liste = new List<Oy>();

            IlSonucu sonuc = SonucBul(secimKod, plaka);
            if (sonuc == null) return liste;

            foreach (Oy oy in sonuc.Oylar)
                if (oy.Oran > 0) liste.Add(oy);

            liste.Sort(delegate(Oy a, Oy b) { return b.Oran.CompareTo(a.Oran); });

            return liste;
        }

        /// <summary>
        /// Bir oy kaleminin ekranda gorunecek adi.
        /// CB secimlerinde aday, MV secimlerinde parti, referandumda secenek.
        /// Tanimli degilse kodun kendisi doner - ekranda bos kutu kalmasin.
        /// </summary>
        public static string OyAdi(string secimKod, string kod)
        {
            ...
            if (secim != null && secim.IsCB)
            {
                ...
                if (aday != null) return aday.Ad;
            }

            if (secim != null && secim.IsREF)
            {
                Secenek secenek = SecenekBul(kod);
                if (secenek != null) return secenek.Ad;
            }

            Parti parti = PartiBul(kod);
            return parti != null ? parti.Ad : kod;
        }
        ...
        /// <summary>
        /// Sahnenin baslik kutusuna sigacak secim adi.
        ///
        /// Secimin tam adi ("MAYIS 2023 MİLLETVEKİLİ SEÇİMİ") kutuya
        /// sigmiyor; sahne tek satirlik kisa bicimi bekliyor.
        /// </summary>
        public static string SecimBasligi(string secimKod)
        {
            Secim secim = SecimBul(secimKod);
            if (secim == null) return "";

            if (secim.IsCB)  return "CUMHURBAŞKANLIĞI SEÇİMİ";
            if (secim.IsREF) return "HALK OYLAMASI";

            return "MİLLETVEKİLİ SEÇİMİ";
        }

        /// <summary>
        /// Dar basliklara sigacak kisa secim adi.
        /// Sol seridin baslik kutusu iki satirlik: ikinci satirda il adi
        /// var, birinci satira "CUMHURBAŞKANLIĞI SEÇİMİ" sigmiyor ve
        /// tasinca il adini ekrandan atiyor.
        /// </summary>
        public static string SecimBasligiKisa(string secimKod)
        {
            Secim secim = SecimBul(secimKod);
            if (secim == null) return "";
        ...
        /// <summary>
        /// Bir oy kaleminin ekran rengi, "R;G;B" biciminde.
        ///
        /// CB secimlerinde kod bir ADAY: adayin kendi rengi yok, bagli
        /// oldugu partinin rengi kullaniliyor (Erdoğan -> AK PARTİ turuncusu).
        /// Partisi yoksa ittifakinin rengine, o da yoksa griye dusuluyor -
        /// ekranda renksiz bir satir kalmasindansa notr bir gri iyidir.
        /// </summary>
        public static string RenkKodu(string secimKod, string kod)
        {
            Secim secim = SecimBul(secimKod);
            ...
            {
                Secenek secenek = SecenekBul(kod);
                if (secenek != null && !string.IsNullOrEmpty(secenek.Renk)) return secenek.Renk;
            }
            ...
            if (secim != null && secim.IsCB)
            {
                ...
                if (aday != null)
                {
                    Parti adayinPartisi = PartiBul(aday.Parti);
                    if (adayinPartisi != null && !string.IsNullOrEmpty(adayinPartisi.Renk))
                        return adayinPartisi.Renk;

                    ittifak = aday.Ittifak;
                }
            }
            ...
            if (parti != null)
            {
                if (!string.IsNullOrEmpty(parti.Renk)) return parti.Renk;
                ittifak = parti.Ittifak;
            }

            Ittifak kayit = IttifakBul(ittifak);
            if (kayit != null && !string.IsNullOrEmpty(kayit.Renk)) return kayit.Renk;

            return "140;140;140";
        }

        /// <summary> Ittifakin ekran rengi, "R;G;B". Tanimli degilse gri. </summary>
        public static string IttifakRengi(string kod)
        {
            Ittifak kayit = IttifakBul(kod);

            return (kayit != null && !string.IsNullOrEmpty(kayit.Renk))
                ? kayit.Renk : "140;140;140";
        }
